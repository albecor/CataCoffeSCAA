...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/updateYourself")
				.access("hasRole('ORGANIZER') or hasRole('ADMIN') or hasRole('TASTER')").antMatchers("/te/**")
				.access("hasRole('TEMPORAL')").antMatchers("/ad/**").access("hasRole('ADMIN')").antMatchers("/or/**")
				.access("hasRole('ORGANIZER')").antMatchers("/ta/**").access("hasRole('TASTER')").antMatchers("/ch/**")
				...
				.rememberMeParameter("remember-me").tokenRepository(tokenRepository).tokenValiditySeconds(1209600);
